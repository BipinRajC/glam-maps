...
export default function RoadReportPage() {
  ...
  const rows = useMemo<RouteRow[]>(() => {
    const all = Object.values(ROUTES).map((route) => ({
      id: route.id,
      glamName: route.glamName,
      glamScore: computeGlamScore(route.integrityEnd),
      visited: passport.includes(route.id),
    }));

    return all.sort((a, b) => {
      if (a.visited !== b.visited) return a.visited ? -1 : 1;
      return b.glamScore - a.glamScore;
    });
  }, [passport]);
            ...
            >
              <div className="flex items-start justify-between mb-5">
                <div>
                  ...
                  <p className="font-inter text-xs mt-1" style={{ color: "#7e77ab" }}>
                    {row.visited ? "Travelled" : "Not visited"}
                  </p>
                </div>
                <div
